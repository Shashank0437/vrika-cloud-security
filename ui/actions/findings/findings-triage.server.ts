...
async function actionResult<T>(
  ...
  try {
    ...
    console.error("Finding triage request failed.", error);
    return {
      ok: false,
...
async function resolveFindingUid({
  ...
  if (typeof resolvedFindingUid !== "string" || !resolvedFindingUid) {
    throw new TriageRequestError(
      "Cannot create finding triage without findingUid.",
    );
  }
...
async function loadLatestFindingTriageNoteImpl(
  ...
  if (!latestNote) {
    throw new TriageRequestError(
      "Could not load the latest finding triage note.",
    );
  }
...
async function updateFindingTriageImpl(input: UpdateFindingTriageInput) {
...
async function loadFindingTriageHistoryImpl(
  triage: FindingTriageSummary,
  page = 1,
  ...
  if (!Array.isArray(result?.data)) {
    throw new TriageRequestError("Could not load triage history.");
  }
...
export async function updateFindingTriage(
  ...
  return actionResult(async () => {
    await updateFindingTriageImpl(input);
  });
}

export async function loadLatestFindingTriageNote(
  triage: FindingTriageSummary,
): Promise<TriageActionResult<FindingTriageLoadedNote>> {
  return actionResult(() => loadLatestFindingTriageNoteImpl(triage));
}
